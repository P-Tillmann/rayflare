...
def RT_wl(i1, wl, n_angles, nx, ny, widths, thetas_in, phis_in, h, xs, ys, nks, surfaces,
          pol, phi_sym, theta_intv, phi_intv, angle_vector, Fr_or_TMM, n_abs_layers, lookuptable, calc_profile,
          depth_spacing, side):
    ...
    def calculate(self, options):
        ...
        if not options['parallel']:
            ...
            allres = Parallel(n_jobs=options['n_jobs'])(
                delayed(parallel_inner)(nks[:, i1], alphas[:, i1], r_a_0, theta, phi,
                                        surfaces, widths, z_pos, I_thresh, pol, nx, ny,
                                        n_reps, xs, ys, randomize, initial_mat, initial_dir, periodic) for
...
def make_profiles_wl(unique_thetas, n_a_in, side, widths,
    ...
    ans = params.loc[dict(layer=prof_layers)].groupby('global_index').map(profile_per_angle,
                                                                          z=z_list, offset=offsets, side=side,
                                                                          nz=nz).drop('coeff')
    ans = ans.fillna(0)
...
class RTSurface:
    def __init__(self, Points):
        ...
        self.zcov = Points[:, 2][
            np.all(np.array([Points[:, 0] == min(Points[:, 0]), Points[:, 1] == min(Points[:, 1])]), axis=0)]
    ...
    def find_area(self):
        ...
        self.area = np.sum((0.5 * np.linalg.norm(xyz[0] - xyz[1], axis=1) * np.linalg.norm(xyz[2] - xyz[1],
                                                                                           axis=1) * np.sin(theta))) / (
                                self.Lx * self.Ly)
...
def single_ray_stack(x, y, nks, alphas, r_a_0, surfaces, widths,
                     z_pos, I_thresh, pol='u', randomize=False, mat_i=0, direction_i=1, periodic=1):
...
def single_interface_check(r_a, d, ni, nj, tri, Lx, Ly, side, z_cov, pol, n_interactions=0, wl=None, Fr_or_TMM=0,
    ...
    while intersect:
        ...
        with np.errstate(divide='ignore',
                         invalid='ignore'):  # there will be divide by 0/multiply by inf - this is fine but gives lots of warnings
...
def single_cell_check(r_a, d, ni, nj, tri, Lx, Ly, side, z_cov, pol, n_interactions=0, wl=None, Fr_or_TMM=0,
                      lookuptable=None):
    decide = {0: decide_RT_Fresnel, 1: decide_RT_TMM}
